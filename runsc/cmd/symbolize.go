...
func (*Symbolize) Usage() string {
	return `symbolize - converts instruction pointers into positions in the runsc source code.
...
func (c *Symbolize) SetFlags(f *flag.FlagSet) {
	f.BoolVar(&c.dumpAll, "all", false, "dumps information on all of the coverage blocks along with their synthetic PCs")
}
...
func (c *Symbolize) Execute(_ context.Context, f *flag.FlagSet, args ...interface{}) subcommands.ExitStatus {
	...
	if !coverage.KcovAvailable() {
		return Errorf("symbolize can only be used when kcov is available.")
	}
	coverage.InitCoverageData()

	if c.dumpAll {
		coverage.PrintAllPCs()
	...
	for scanner.Scan() {
		...
		if strings.HasPrefix(str, "0x") {
			pc, err = strconv.ParseUint(scanner.Text(), 0 /* base */, 64 /* bitSize */)
		} else {
			pc, err = strconv.ParseUint(scanner.Text(), 16 /* base */, 64 /* bitSize */)
		}
		if err != nil {
			break
		}
		file, err = coverage.FileFromIndex(int(pc >> 16))
		if err != nil {
			break
		}
		block, err = coverage.BlockFromIndex(file, int(pc&((1<<16)-1)))
		if err != nil {
			break
		}
		fmt.Printf("%#x\n", pc)
		fmt.Printf("%s:%d,%d-%d,%d\n", file, block.FirstLine, block.FirstCol, block.LastLine, block.LastCol)
	}
	if err != nil {
